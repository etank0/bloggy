...
export default function PostForm({ post }) {
  ...
  const submit = async (data) => {
    try {
      setLoading(true);
      const imageFile = Array.isArray(data?.image) ? data?.image[0] : data?.image || null;
      if (post) {
        ...
        const file =
          imageFile && (await storageService.uploadFile(imageFile));
          ...
          >
              ...
              <div className='mb-2'>
                <Input
                  ...
                  {...register('title', {
                    ...
                    maxLength: { value: 255, message: 'Title must be at most 255 characters long!' },
                  })}
                />
                {errors.title && <p className="text-red-400 text-xs pl-2">{errors.title.message}</p>}
              </div>
              <div className='mb-2'>
                <Input
                  label="Slug: "
                  ...
                  {...register('slug', {
                    ...
                    minLength: { value: 10, message: 'Slug must be at least 10 characters long!' },
                    maxLength: { value: 36, message: 'Slug must be at most 36 characters long!' },
                  })}
                  onInput={(e) => {
                ...
                />
                {errors.slug && <p className="text-red-400 text-xs pl-2">{errors.slug.message}</p>}
              </div>
              <RealtimeEditor
                label="Content: "
                name="content"
            ...
            <div className="w-[95%] md:w-1/4 px-2">
              <div className='mb-2'>
                ...
                />
                {errors.status && <p className="text-red-400 text-xs pl-2">{errors.status.message}</p>}
              </div>
              {/* <Input
              ...
              <div className='mb-2'>
                ...
                />
                {errors.image && <p className="text-red-400 text-xs pl-2">{errors.image.message}</p>}
              </div>
              {post && (
